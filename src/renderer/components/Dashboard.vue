    ...
    <div class="home-container">
        ...
        <div class="wallet-info">
            <p class="font-regular">
                {{$t('sharedWalletHome.address')}}:
                <span class="font-gray">{{address}}</span>
            ...
            <div class="right-half">
                <div class="completed-tx">
                    ...
                    <div class="check-more" v-if="completedTx.length > 6" @click="checkMoreTx">
                        ...
                        <i
                            class="fa fa-chevron-right icon-arrow"
                        ></i>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
            ...
            try {
                ...
                for (const t of txlist) {
                    for (const tx of t.transfers) {
                        ...
                        let amount =
                            asset === "ONT" ? parseInt(tx.amount) : tx.amount;
        ...
        getBalance() {
            return this.$store
                .dispatch("getNativeBalance", { address: this.address })
                .then(res => {
                    if (!res) {
        ...
        getOep4Balances() {
            ...
            this.$store
                .dispatch("fetchTokenBalances", { address: this.address })
                .then(res => {
                    if (!res) {
                        this.$message.error(this.$t("dashboard.getBalanceErr"));
                    }
        ...
        redeemOng() {
            ...
            if (this.currentWallet.key) {
                ...
                this.$router.push({
                    path: "/commonWalletRedeem/hardwareWallet"
                });
            }
        },
        goBack() {
        ...
        toCrossChainTransfer() {
            this.$router.push({name: 'CrossChainTransfer'})
        },
        toDevTool() {
            this.$router.push({name: 'CrossChainDevTool'})
        }
    }
};
</script>
